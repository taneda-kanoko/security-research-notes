...
## Run1 検証結果（2026-05-16）

| 仮説 | Verdict | 条件 |
|:---|:---|:---|
| H1 | Partial Support | 監視インフラあれば独立検知可。H2経由でも確認可 |
| H2 | Confirmed Support | 漏洩パッケージ名を把握していれば API 1コールで確定 |
| H3 | Partial → Confirmed | 単体は偽陽性あり。H2との組み合わせで確定シグナル |

**核心的発見**: 1ビットの前提条件（漏洩情報の把握）が
検知率100%と0%を完全に分断する。
技術的インフラではなく情報優位の有無が構造的脆弱性の本体。
...
- [Run 2: SBOM を最小検知条件として検証](npm-squatting-sbom-detection-2026.md) — Run 1 で発見した「1ビット前提条件問題」を SBOM 保有によって解消できるかを検証した続編。
